...
router.post("/",  [ authMiddleware, roleMiddleware([ "ADMIN" ]) ], (req: Request, res: Response) => enrollmentController.createEnrollment(req, res));
...
router.patch("/:id",  [ authMiddleware, roleMiddleware([ "ADMIN" ]) ], (req: Request, res: Response) => enrollmentController.updateEnrollment(req, res));
